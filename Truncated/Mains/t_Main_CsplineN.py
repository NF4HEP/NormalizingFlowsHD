...
def MixtureGaussian(ncomp,ndims,seed=0):
    targ_dist,min_conds,max_conds = MixtureDistributions.UnimodalTruncatedMixMultiNormal1(ndims,seed=0)
    return targ_dist,min_conds,max_conds

### Initialize number of components ###
ncomp=3

### Initialize hyperparameters lists ###
ndims_list=[4,8,16,32,64,100]
...
hidden_layers_list=[[128,128,128],[256,256,256],[512,512,512]]
seeds_list = [0]
n_displays=1
...
for ndims in ndims_list:
    ...
    print("===========\n")
    start=timer()
    targ_dist,min_conds,max_conds=MixtureGaussian(ncomp,ndims,seed=0)
    #targ_dist,SoftClipBijector=MixtureDistributions.TruncateDistribution(targ_dist,ndims,seed=seed_dist)
    X_data_test=targ_dist.sample(ntest_samples,seed=seed_test).numpy()
    #Plotters.marginal_plot(X_data_test,X_data_test,'.',ndims)
    #print('TESTPLOT')
    end=timer()
                                                ...
                                                for corr in corr_uncorr_list:
                                                    ...
                                                    try:
                                                        if to_run:
                                                            ...
                                                            #print(nf_dist)
                                                            ##soft clipping
                                                            SoftClipBijector=tfb.SoftClip(low=min_conds, high=max_conds,hinge_softness=.0001)
                                                            #print('heeellloooo  11')
                                                            #print(SoftClipBijector)
                                                            #print(len(min_conds))
                                                            nf_dist=tfd.TransformedDistribution(nf_dist,SoftClipBijector) 
                                                            print('heeellloooo')
                                                            #print(nf_dist)
