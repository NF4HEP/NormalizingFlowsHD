...
mother_output_dir='../results/MAFN_3/'
try:
...
### Run loop  ###
run_number = 0
n_runs = len(ndims_list)*len(seeds_list)*len(nsamples_list)*len(corr_uncorr_list)*len(activation_list)*len(eps_regularisers)*len(regulariser_list)*len(bijectors_list)*len(nbijectors_list)*len(spline_knots_list)*len(range_min_list)*len(batch_size_list)*3
for ndims in ndims_list:
    if ndims < 200:
        hidden_layers_list=[[2*ndims,2*ndims,2*ndims],[4*ndims,4*ndims,4*ndims],[512,512,512]]
    elif ndims == 200:
        hidden_layers_list=[[2*ndims,2*ndims,2*ndims],[512,512,512],[4*ndims,4*ndims,4*ndims]]
    else:
        hidden_layers_list=[[512,512,512],[2*ndims,2*ndims,2*ndims],[4*ndims,4*ndims,4*ndims]]
