...
with h5py.File(events_dataset_path, 'r') as hdf:
    ...
    try:
        events = np.array(hdf['Z+3j'][:]).astype(np.float32)
    except:
        raise Exception("Z+3j not found in the dataset.")
...
def CoordTransforms(events):
    ...
    pt_jet2=np.sqrt(px_jet2**2+py_jet2**2)
    rapid_jet2=.5*np.log((E_jet2+pz_jet2)/(E_jet2-pz_jet2))
    azim_jet2=np.sign(py_jet2)*np.arccos(px_jet2/(np.sqrt(px_jet2**2+py_jet2**2)))

    m_jet2_sq=E_jet2**2-(px_jet2**2+py_jet2**2+pz_jet2**2)
    print('max mjet2')
    print(np.max(m_jet2_sq,axis=0))
    print('min mjet2')
    print(np.min(m_jet2_sq,axis=0))
 
    
    
    m_jet2=np.sqrt(E_jet2**2-(px_jet2**2+py_jet2**2+pz_jet2**2))
    mt_jet2=np.sqrt(m_jet2**2+pt_jet2**2)

    spher_jet2=np.array([pt_jet2,rapid_jet2,azim_jet2,m_jet2])
    print(spher_jet2)
    print(np.shape(spher_jet2))
    spher_jet2=np.transpose(spher_jet2)
    print(spher_jet2)
    print(np.shape(spher_jet2))


    print('max jet2')
    print(np.max(spher_jet2,axis=0))
    print('min jet2')
    print(np.min(spher_jet2,axis=0))
    print('min E-pz')
    print(np.min(E_jet2-pz_jet2,axis=0))
    #name='corner_plot_jet1.png'
    #cornerplotter(spher_jet1,name)
    ...
    pt_jet3=np.sqrt(px_jet3**2+py_jet3**2)
    rapid_jet3=.5*np.log((E_jet3+pz_jet3)/(E_jet3-pz_jet3))
    azim_jet3=np.sign(py_jet3)*np.arccos(px_jet3/(np.sqrt(px_jet3**2+py_jet3**2)))

    m_jet3_sq=E_jet3**2-(px_jet3**2+py_jet3**2+pz_jet3**2)
    print('max mjet3')
    print(np.max(m_jet3_sq,axis=0))
    print('min mjet3')
    print(np.min(m_jet3_sq,axis=0))
 
    
    
    m_jet3=np.sqrt(E_jet3**2-(px_jet3**2+py_jet3**2+pz_jet3**2))
    mt_jet3=np.sqrt(m_jet3**2+pt_jet3**2)

    spher_jet3=np.array([pt_jet3,rapid_jet3,azim_jet3,m_jet3])
    print(spher_jet3)
    print(np.shape(spher_jet3))
    spher_jet3=np.transpose(spher_jet3)
    print(spher_jet3)
    print(np.shape(spher_jet3))


    print('max jet3')
    print(np.max(spher_jet3,axis=0))
    print('min jet3')
    print(np.min(spher_jet3,axis=0))
    print('min E-pz')
    print(np.min(E_jet3-pz_jet3,axis=0))
    #name='corner_plot_jet1.png'
    #cornerplotter(spher_jet1,name)
    
    spher_all=np.concatenate((spher_mu1,spher_mu2,spher_jet1,spher_jet2,spher_jet3),axis=1)
...
def Undo_Preprocess_1(spher_all):
...
def BackToCartesian(LHCcoord):
    ...
    pt_jet2=LHCcoord[:,10+0]
    rapid_jet2=LHCcoord[:,10+1]
    azim_jet2=LHCcoord[:,10+2]
    m_jet2=LHCcoord[:,10+3]
    
    
    
    mt_jet2=np.sqrt(pt_jet2**2+m_jet2**2)
    
    px_jet2=pt_jet2*np.cos(azim_jet2)
    py_jet2=pt_jet2*np.sin(azim_jet2)
    pz_jet2=mt_jet2*np.sinh(rapid_jet2)
    E_jet2=mt_jet2*np.cosh(rapid_jet2)
    
    coord_jet2=np.array([E_jet2,px_jet2,py_jet2,pz_jet2])
    print(coord_jet2)
    print(np.shape(coord_jet2))
    coord_jet2=np.transpose(coord_jet2)
    
    
    #jets3
    
    
    pt_jet3=LHCcoord[:,14+0]
    rapid_jet3=LHCcoord[:,14+1]
    azim_jet3=LHCcoord[:,14+2]
    m_jet3=LHCcoord[:,14+3]
    
    
    
    mt_jet3=np.sqrt(pt_jet3**2+m_jet3**2)
    
    px_jet3=pt_jet3*np.cos(azim_jet3)
    py_jet3=pt_jet3*np.sin(azim_jet3)
    pz_jet3=mt_jet3*np.sinh(rapid_jet3)
    E_jet3=mt_jet3*np.cosh(rapid_jet3)
    
    coord_jet3=np.array([E_jet3,px_jet3,py_jet3,pz_jet3])
    print(coord_jet3)
    print(np.shape(coord_jet3))
    coord_jet3=np.transpose(coord_jet3)
    
    
    
    coord_all=np.concatenate((coord_mu1,coord_mu2,coord_jet1,coord_jet2,coord_jet3),axis=1)
...
def SaveCoord(spher_all):
    ...
    file = h5py.File('LHCCoordZjet3.h5', 'w')
    file.create_dataset('LHCCoordZjet3', data=spher_all)
    file.close()
    return
...
SaveCoord(spher_all)
name='corner_plot_sphercoord_jet3.png'
cornerplotter(spher_all,name)


cart_coord=BackToCartesian(spher_all_orig)
name='cart_vs_undo_jet3.png'
cornerplotter_comp(events[:100000,:],cart_coord[:100000,:],name)



spher_all_prep=Preprocess_1(spher_all_orig)
name='corner_plot_sphercoord_preprocessed_nomin_jet3.png'
cornerplotter(spher_all[:1000000,:],name)


spher_all_undo=Undo_Preprocess_1(spher_all_prep)
name='corner_plot_sphercoord_postprocessed_nomin_jet3.png'
cornerplotter(spher_all_undo[:1000000,:],name)


name='orig_vs_undo_nomin_jet3.png'
cornerplotter_comp(spher_all_orig[:1000000,:],spher_all_undo[:1000000,:],name)

cart_coord_afterundo=BackToCartesian(spher_all_undo)
name='cart_vs_undo_afterundo_nomin_jet3.png'
cornerplotter_comp(events[:1000000,:],cart_coord[:1000000,:],name)
name='cart_vs_undo_afterundo_marginal_nomin_jet3.png'
