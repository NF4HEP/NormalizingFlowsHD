...
class GeneralLorentzTransformBijector(tfb.Bijector):
    ...
    def _forward(self, x):
        ...
        The transformation is chosen by a neural network taking the 4-momentum vector of the first particle as input.
        The transformation is then applied to all particles in the event but the first one.
        ...
        parameters_extended = tf.tile(parameters_expanded, [1, self.n_particles - 1, 1]) # shape: (n_samples, n_particles - 1, 6)
        ...
        x_transformed = lorentz_transform(x[:, 4:], rapidity, phi, theta, alpha, beta, gamma) # transform all particles but the first one
        x_transformed = tf.reshape(x_transformed, [-1, 4*(self.n_particles-1)])  # reshape the transformed particles 
        # Concatenate the first particle (untransformed) with the transformed ones
        x_transformed_output = tf.concat([x[:, :4], x_transformed], axis=1)
        return x_transformed_output
    ...
    def _inverse(self, y):
        ...
        beta = parameters_extended[..., 4]  # Euler angle 2: rotation around y-axis. shape (n_events,n_particles)
        gamma = parameters_extended[..., 5]  # Euler angle 3: rotation around new z-axis. shape (n_events,n_particles)
        # Apply the inverse of the chosen Lorentz transformation to all particles except the first one
        y_transformed = inverse_lorentz_transform(y[:, 4:], rapidity, phi, theta, alpha, beta, gamma)
        y_transformed = tf.reshape(y_transformed, [-1, 4*(self.n_particles-1)])  # reshape the transformed particles 
        # Concatenate the first particle (untransformed) with the transformed ones
        y_transformed_output = tf.concat([y[:, :4], y_transformed], axis=1)
        return y_transformed_output
